...
def get_bag_feats(csv_file_df, args):
    if args.dataset == "TCGA-lung-default":
        feats_csv_path = (
            "datasets/tcga-dataset/tcga_lung_data_feats/"
            + csv_file_df.iloc[0].split("/")[1]
            + ".csv"
        )
    else:
...
def train(train_df, milnet, criterion, optimizer, args):
    ...
    for i in range(len(train_df)):
        ...
        sys.stdout.write(
            "\r Training bag [%d/%d] bag loss: %.4f" % (i, len(train_df), loss.item())
        )
...
def dropout_patches(feats, p):
    idx = np.random.choice(
        np.arange(feats.shape[0]), int(feats.shape[0] * (1 - p)), replace=False
    )
...
def test(test_df, milnet, criterion, optimizer, args):
    ...
    with torch.no_grad():
        for i in range(len(test_df)):
            ...
            total_loss = total_loss + loss.item()
            sys.stdout.write(
                "\r Testing bag [%d/%d] bag loss: %.4f" % (i, len(test_df), loss.item())
            )
            ...
            if args.average:
                test_predictions.extend(
                    [
                        (
                            0.5 * torch.sigmoid(max_prediction)
                ...
                test_predictions.extend(
                    [
                        (
                            0.0 * torch.sigmoid(max_prediction)
                            + 1.0 * torch.sigmoid(bag_prediction)
                        )
                        .squeeze()
                        .cpu()
                        .numpy()
...
def main():
    ...
    parser.add_argument(
        ...
        default=512,
        type=int,
        help="Dimension of the feature size [512]",
    )
    parser.add_argument(
        "--lr", default=0.0002, type=float, help="Initial learning rate [0.0002]"
    )
    parser.add_argument(
        "--num_epochs",
        default=200,
        type=int,
        help="Number of total training epochs [40|200]",
    )
    parser.add_argument(
        "--gpu_index", type=int, nargs="+", default=(0,), help="GPU ID(s) [0]"
    )
    parser.add_argument(
        "--weight_decay", default=5e-3, type=float, help="Weight decay [5e-3]"
    )
    parser.add_argument(
        "--dataset", default="TCGA-lung-default", type=str, help="Dataset folder name"
    )
    parser.add_argument(
        "--split", default=0.2, type=float, help="Training/Validation split [0.2]"
    )
    parser.add_argument("--model", default="dsmil", type=str, help="MIL model [dsmil]")
    parser.add_argument(
        "--dropout_patch", default=0, type=float, help="Patch dropout rate [0]"
    )
    parser.add_argument(
        "--dropout_node", default=0, type=float, help="Bag classifier dropout rate [0]"
    )
    parser.add_argument(
        "--non_linearity",
        default=1,
        type=float,
        help="Additional nonlinear operation [0]",
    )
    parser.add_argument(
        "--average",
        type=bool,
    ...
    i_classifier = mil.FCLayer(
        in_size=args.feats_size, out_size=args.num_classes
    ).cuda()
    ...
    for epoch in range(1, args.num_epochs):
        ...
        train_loss_bag = train(
            train_path, milnet, criterion, optimizer, args
        )  # iterate all bags
